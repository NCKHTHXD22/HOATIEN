...
function CategoryMemberPanel({ cat, followers, onDelete }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/api/broadcast/groups/${cat.group_id}/members`).then((r) => r.data),
    enabled: open,
  })

  const { data: pendingData } = useQuery({
    queryKey: ['zalo-pending', cat.group_id],
    queryFn: () => api.get(`/api/broadcast/groups/${cat.group_id}/pending`).then((r) => r.data),
  ...
  const addMutation = useMutation({
    mutationFn: (follower) =>
      api.post(`/api/broadcast/groups/${cat.group_id}/members`, {
  ...
  const deleteMutation = useMutation({
    mutationFn: (memberId) => api.delete(`/api/broadcast/groups/members/${memberId}`).then((r) => r.data),
    onSuccess: () => {
  ...
  const syncMutation = useMutation({
    mutationFn: () => api.post(`/api/broadcast/groups/${cat.group_id}/members/sync`).then((r) => r.data),
    onSuccess: (data) => {
...
export default function SettingsPage() {
  ...
  const createGroupMutation = useMutation({
    mutationFn: (data) => api.post('/api/broadcast/groups/create-zalo', data).then((r) => r.data),
    onSuccess: () => {
  ...
  const categories = catsData?.groups ?? []
